/**
 * Lists image files in two subfolders under public/assets/sotilasmerkisto and writes
 * src/data/tacticalSignsPaths.ts. Run from project root:
 *   node scripts/list-tactical-signs.cjs
 *
 * Folder structure:
 *   public/assets/sotilasmerkisto/Sotilasmerkistö/   → Sotilasmerkistö button
 *   public/assets/sotilasmerkisto/Joukkojen koko/   → Joukkojen koko button
 * Filename (without extension) = correct answer term. Re-run after adding/renaming images.
 */
const fs = require('fs')
const path = require('path')

const TACTICAL_BASE = path.join(__dirname, '..', 'public', 'assets', 'sotilasmerkisto')
...
function walkDir(dir, pathPrefix, acc) {
  ...
  for (const e of entries) {
    ...
    if (e.isDirectory()) {
      ...
      acc.push('/assets/sotilasmerkisto/' + rel.replace(/\\/g, '/'))
    }
  }
}
